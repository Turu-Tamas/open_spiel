...
const GameType kGameType{/*short_name=*/"hungarian_tarok",
                         ...
                         /*parameter_specification=*/
                          ...
                          {"starting_player", GameParameter(kDefaultStartingPlayer)}}};
...
class HungariantarokObserver : public Observer {
 public:
  HungariantarokObserver(IIGObservationType iig_obs_type)
  ...
  static void WriteObservingPlayer(const HungariantarokState& state, int player,
                                   Allocator* allocator) {
  ...
  static void WriteSinglePlayerCard(const HungariantarokState& state, int player,
                                    Allocator* allocator) {
  ...
  static void WriteAllPlayerCards(const HungariantarokState& state,
                                  Allocator* allocator) {
  ...
  static void WriteCommunityCard(const HungariantarokState& state,
                                 Allocator* allocator) {
  ...
  static void WriteBettingSequence(const HungariantarokState& state,
                                   Allocator* allocator) {
  ...
  static void WritePotContribution(const HungariantarokState& state,
                                   Allocator* allocator) {
  ...
  void WriteTensor(const State& observed_state, int player,
    ...
    auto& state = open_spiel::down_cast<const HungariantarokState&>(observed_state);
...
HungariantarokState::HungariantarokState(std::shared_ptr<const Game> game, bool action_mapping,
...
std::string HungariantarokState::InformationStateString(Player player) const {
  const HungariantarokGame& game = open_spiel::down_cast<const HungariantarokGame&>(*game_);
...
void HungariantarokState::InformationStateTensor(Player player,
...
int HungariantarokState::NextPlayer() const {
  ...
  SpielFatalError("Error in HungariantarokState::NextPlayer(), should not get here.");
}

int HungariantarokState::RankHand(Player player) const {
...
void HungariantarokState::ResolveWinner() {
...
void HungariantarokState::SequenceAppendMove(int move) {
  if (round_ == 1) {
...
void HungariantarokState::Ante(Player player, int amount) {
  pot_ += amount;
  ante_[player] += amount;
  money_[player] -= amount;
}

std::vector<int> HungariantarokState::padded_betting_sequence() const {
...
void HungariantarokState::SetPrivate(Player player, Action move) {
...
std::unique_ptr<State> HungariantarokState::ResampleFromInfostate(
...
int HungariantarokState::NumObservableCards() const {
  return suit_isomorphism_ ? deck_.size() / 2 : deck_.size();
}

int HungariantarokState::MaxBetsPerRound() const { return 3 * num_players_ - 2; }

void HungariantarokState::SetPrivateCards(const std::vector<int>& new_private_cards) {
...
HungariantarokGame::HungariantarokGame(const GameParameters& params)
  ...
  default_observer_ = std::make_shared<HungariantarokObserver>(kDefaultObsType);
  info_state_observer_ = std::make_shared<HungariantarokObserver>(kInfoStateObsType);
}

std::unique_ptr<State> HungariantarokGame::NewInitialState() const {
  return absl::make_unique<HungariantarokState>(
      shared_from_this(),
...
int HungariantarokGame::MaxChanceOutcomes() const {
  if (suit_isomorphism_) {
...
std::vector<int> HungariantarokGame::InformationStateTensorShape() const {
...
double HungariantarokGame::MaxUtility() const {
...
double HungariantarokGame::MinUtility() const {
  // In poker, the utility is defined as the money a player has at the end of
  // the game minus then money the player had before starting the game.
...
TabularPolicy GetAlwaysFoldPolicy(const Game& game) {
  SPIEL_CHECK_TRUE(
      dynamic_cast<HungariantarokGame*>(const_cast<Game*>(&game)) != nullptr);
