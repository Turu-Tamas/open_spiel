...
namespace hungarian_tarok {
    BiddingPhase::BiddingPhase(GameData &game_data) : GamePhase(game_data) {
        has_honour_[game_data_.deck_[kSkiz]] = true;
        has_honour_[game_data_.deck_[kPagat]] = true;
        has_honour_[game_data_.deck_[kXXI]] = true;
    }
    ...
    void BiddingPhase::DoApplyAction(Action action) {
        ...
        if (action == kActionStandardBid) {
            ...
            game_data_.declarer_ = current_player_;
    ...
    std::unique_ptr<GamePhase> BiddingPhase::NextPhase() const {
        ...
        game_data_.full_bid_ = (bidder_count == 3); // all three honours bid
        game_data_.winning_bid_ = lowest_bid_;
        return std::make_unique<DealTalonPhase>(game_data_);
    }
