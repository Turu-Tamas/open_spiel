...
class BiddingPhase final : public GamePhase {
 public:
    ...
    bool PhaseOver() const override { return current_player_ == kTerminalPlayerId; }
...
class AnnouncementsPhase final : public GamePhase {
        ...
        for (int i = 0; i < kNumAnnouncementTypes; ++i) {
            ...
            switch (type) {
                case AnnouncementType::kTuletroa:
                    if (CanAnnounceTuletroa()) actions.push_back(AnnouncementAction::AnnounceAction(type));
                    break;
                case AnnouncementType::kEightTaroks:
                    if (tarok_counts_[current_player_] == 8) actions.push_back(AnnouncementAction::AnnounceAction(type));
                    break;
                case AnnouncementType::kNineTaroks:
                    if (tarok_counts_[current_player_] == 9) actions.push_back(AnnouncementAction::AnnounceAction(type));
                    break;
                default:
 ...
 private:
    ...
    GameData::AnnouncementSide& CurrentSide() {
        return IsDeclarerSidePlayer(current_player_) ? game_data().declarer_side_ : game_data().opponents_side_;
    }
    GameData::AnnouncementSide& OtherSide() {
        return IsDeclarerSidePlayer(current_player_) ? game_data().opponents_side_ : game_data().declarer_side_;
    }
    ...
    bool CanAnnounceTuletroa() const {
        ...
        if (current_player_ == game_data().declarer_ && first_round_) {
            ...
            return game_data().deck_[kXXI] == game_data().declarer_ && game_data().deck_[kSkiz] == game_data().declarer_;
        }
        if (game_data().partner_.has_value() && current_player_ == *game_data().partner_ && first_round_) {
            // In the first round, tuletroa from partner means XXI or Skiz in hand.
            return game_data().deck_[kXXI] == *game_data().partner_ || game_data().deck_[kSkiz] == *game_data().partner_;
        }
        return true;
    }
...
std::string SetupPhase::ActionToString(Player player, Action action) const {
    SPIEL_CHECK_GE(action, 0);
    SPIEL_CHECK_LT(action, kNumPlayers);
    return absl::StrCat("Deal card ", CardToString(current_card_), " to player ", action);
}
