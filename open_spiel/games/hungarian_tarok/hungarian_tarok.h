...
class HungariantarokObserver;

enum ActionType { kFold = 0, kCall = 1, kRaise = 2 };

class HungariantarokState : public State {
 public:
  explicit HungariantarokState(std::shared_ptr<const Game> game, bool action_mapping,
 ...
 private:
  friend class HungariantarokObserver;
...
class HungariantarokGame : public Game {
 public:
  ...
  std::shared_ptr<HungariantarokObserver> default_observer_;
  std::shared_ptr<HungariantarokObserver> info_state_observer_;
